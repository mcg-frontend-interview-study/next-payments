...
export const Form = () => {
  ...
  const fields: Array<React.ReactNode> = [
    <Field key={0} title="결제할 카드 번호를 입력해 주세요" description="본인 명의의 카드만 결제 가능합니다.">
      <CardNumbers cardNumbers={cardNumbers} nextStep="cardBrand" handleNext={handleNext} />
    </Field>,
    <Field key={1} title="카드사를 선택해 주세요" description="현재 국내 카드사만 가능합니다.">
      <CardBrand cardBrand={cardBrand} nextStep="expirationDate" handleNext={handleNext} />
    </Field>,
    <Field key={2} title="카드 유효기간을 입력해 주세요" description="월/년도(MMYY)를 순서대로 입력해 주세요.">
      <ExpirationDate expirationDate={expirationDate} nextStep="cardName" handleNext={handleNext} />
    </Field>,
    <Field key={3} title="카드 소유자 이름을 입력해 주세요">
      <CardName cardName={cardName} nextStep="cvc" handleNext={handleNext} />
    </Field>,
    <Field key={4} title="CVC 번호를 입력해 주세요">
      <CVC cvc={cvc} nextStep="password" handleNext={handleNext} />
    </Field>,
    <Field key={5} title="비밀번호를 입력해 주세요" description="앞의 2자리를 입력해주세요">
